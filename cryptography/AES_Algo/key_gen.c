 ...
 * Output: 1CDFAABAB7B9BA7E0EE939035F8165AA
 */
...
unsigned char *key_generator()
{
    ...
    unsigned char *key = (unsigned char *)malloc(BLOCK_SIZE); // Allocate memory for 128-bit key
    if (key == NULL)
    {
    ...
    if (RAND_bytes(key, BLOCK_SIZE) != 1)
    {
